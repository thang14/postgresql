...
postgresql_configure_replication_parameters() {
    ...
    if (( POSTGRESQL_NUM_SYNCHRONOUS_REPLICAS > 0 )); then
        ...
        postgresql_set_property "synchronous_standby_names" "FIRST ${POSTGRESQL_NUM_SYNCHRONOUS_REPLICAS} (\"${POSTGRESQL_CLUSTER_APP_NAME}\")"
    fi
}

########################
# Change postgresql.conf by setting shared_preload_libraries
# Globals:
#   POSTGRESQL_*
# Arguments:
#   None
...
#   None
#########################
postgresql_configure_libraries() {
    postgresql_info "Configuring libraries"
    postgresql_set_property "shared_preload_libraries" "wal2json"
}
...
postgresql_configure_fsync() {
    ...
    postgresql_set_property "wal_sync_method" "fsync"
}
...
postgresql_initialize() {
    ...
    if ! is_dir_empty "$POSTGRESQL_DATA_DIR"; then
        ...
        is_boolean_yes "$create_conf_file" && postgresql_configure_libraries
